...
return new class extends Migration {
    ...
    {
        // 1) Spalte sicherstellen (falls in alten Läufen noch nicht angelegt)
        ...
        // 2) FK(s) defensiv entfernen, falls es schon einen gibt
        $this->dropFkIfExists('projekte', 'verantwortlicher_id');

        // 3) Spaltentyp korrigieren -> BIGINT UNSIGNED NULL
        DB::statement('ALTER TABLE `projekte` MODIFY `verantwortlicher_id` BIGINT UNSIGNED NULL');

        // 4) FK neu setzen (bei Löschen des Mitarbeiters => SET NULL)
        Schema::table('projekte', function (Blueprint $table) {
    ...
    public function down(): void
    {
        ...
        // Falls du die Spalte im Down entfernen willst, auskommentieren:
        // Schema::table('projekte', function (Blueprint $table) {
