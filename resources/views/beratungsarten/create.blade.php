@extends('layouts.app')
@section('title','Neue Beratungsart')
@section('content')
<h2 class="text-2xl font-bold mb-4">Neue Beratungsart</h2>
<form action="{{ route('beratungsarten.store') }}" method="POST" class="bg-white p-6 rounded shadow-sm max-w-lg">
  @csrf
  <label class="block text-sm mb-1">Code (3 Zeichen)</label>
  <input name="Code" value="{{ old('Code') }}" class="border rounded w-full px-3 py-2 mb-3">
  <label class="block text-sm mb-1">Bezeichnung</label>
  <input name="Bezeichnung" value="{{ old('Bezeichnung') }}" class="border rounded w-full px-3 py-2 mb-4">
