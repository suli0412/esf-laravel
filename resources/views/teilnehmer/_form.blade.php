@php
    ...
    /* NEU: Kategorien-Auswahl statt Freitext */
    $staatsKat = ['EU','EWR','Drittstaat','Asylberechtigt','Asylwerbende:r','Sonstiges'];

    /* kleine Helper-Funktion fürs Datumsfeld */
    $geburtsdatumValue = old('Geburtsdatum');
    if (!$geburtsdatumValue && isset($teilnehmer->Geburtsdatum) && $teilnehmer->Geburtsdatum) {
        try {
            $geburtsdatumValue = \Illuminate\Support\Carbon::parse($teilnehmer->Geburtsdatum)->format('Y-m-d');
        } catch (\Throwable $e) {
            $geburtsdatumValue = $teilnehmer->Geburtsdatum; // Fallback, falls bereits korrekt
        }
    }
@endphp
...
<div class="grid grid-cols-12 gap-6">
    ...
    <section class="col-span-12 bg-white rounded-xl shadow-sm p-6">
        ...
        <div class="grid grid-cols-12 gap-4">
            <x-field class="col-span-4" label="Nachname">
                ...
                @error('Nachname') <p class="text-red-600 text-sm mt-1">{{ $message }}</p> @enderror
            </x-field>
            ...
            <x-field class="col-span-4" label="Vorname">
                ...
                @error('Vorname') <p class="text-red-600 text-sm mt-1">{{ $message }}</p> @enderror
            </x-field>
            ...
            <x-field class="col-span-4" label="Geschlecht">
                ...
                @error('Geschlecht') <p class="text-red-600 text-sm mt-1">{{ $message }}</p> @enderror
            </x-field>
            ...
            <x-field class="col-span-3" label="SVN">
                ...
                @error('SVN') <p class="text-red-600 text-sm mt-1">{{ $message }}</p> @enderror
            </x-field>
            ...
            <x-field class="col-span-3" label="Geburtsdatum">
                <input type="date" name="Geburtsdatum" value="{{ $geburtsdatumValue }}" class="w-full border rounded-lg px-3 py-2">
                @error('Geburtsdatum') <p class="text-red-600 text-sm mt-1">{{ $message }}</p> @enderror
            </x-field>
            ...
            {{-- NEU: Kategorie als Select (kein "?"-Wert) --}}
            <x-field class="col-span-3" label="Staatszugehörigkeit Kategorie">
                <select name="Staatszugehörigkeit_Kategorie" class="w-full border rounded-lg px-3 py-2">
                    <option value="">— bitte wählen —</option>
                    @foreach ($staatsKat as $kat)
                        <option value="{{ $kat }}" @selected(old('Staatszugehörigkeit_Kategorie', $teilnehmer->Staatszugehörigkeit_Kategorie ?? '')===$kat)>{{ $kat }}</option>
                    @endforeach
                </select>
                @error('Staatszugehörigkeit_Kategorie') <p class="text-red-600 text-sm mt-1">{{ $message }}</p> @enderror
            </x-field>

            {{-- NEU: Aufenthaltsstatus als Select (kein "?"-Wert) --}}
            <x-field class="col-span-3" label="Aufenthaltsstatus">
                <select name="Aufenthaltsstatus" class="w-full border rounded-lg px-3 py-2">
                    <option value="">— bitte wählen —</option>
                    <option value="Staatsbürgerin"   @selected(old('Aufenthaltsstatus', $teilnehmer->Aufenthaltsstatus ?? '')==='Staatsbürgerin')>Staatsbürger:in</option>
                    <option value="Aufenthaltstitel" @selected(old('Aufenthaltsstatus', $teilnehmer->Aufenthaltsstatus ?? '')==='Aufenthaltstitel')>Aufenthaltstitel</option>
                    <option value="Asylberechtigt"   @selected(old('Aufenthaltsstatus', $teilnehmer->Aufenthaltsstatus ?? '')==='Asylberechtigt')>Asylberechtigt</option>
                    <option value="Asylwerbend"      @selected(old('Aufenthaltsstatus', $teilnehmer->Aufenthaltsstatus ?? '')==='Asylwerbend')>Asylwerbend</option>
                    <option value="Sonstiges"        @selected(old('Aufenthaltsstatus', $teilnehmer->Aufenthaltsstatus ?? '')==='Sonstiges')>Sonstiges</option>
                </select>
                @error('Aufenthaltsstatus') <p class="text-red-600 text-sm mt-1">{{ $message }}</p> @enderror
            </x-field>
        </div>
    </section>
    ...
    <section class="col-span-12 bg-white rounded-xl shadow-sm p-6">
        ...
        <x-field class="col-span-3" :label="$label">
            ...
            <option value="">— bitte wählen —</option>
            <option value="1" {{ (string)old($name, isset($teilnehmer)?(string)($teilnehmer->$name):'') === '1' ? 'selected' : '' }}>Ja</option>
            <option value="0" {{ (string)old($name, isset($teilnehmer)?(string)($teilnehmer->$name):'') === '0' ? 'selected' : '' }}>Nein</option>
            </select>
        ...
        @endforeach
            ...
            <x-field class="col-span-3" label="Bildungshintergrund">
                ...
                @error('Bildungshintergrund') <p class="text-red-600 text-sm mt-1">{{ $message }}</p> @enderror
            </x-field>
        </div>
    </section>
    ...
    <section class="col-span-12 bg-white rounded-xl shadow-sm p-6">
        ...
        <div class="grid grid-cols-12 gap-4 items-center">
            ...
            <x-field class="col-span-6" label="PAZ">
                ...
                @error('PAZ') <p class="text-red-600 text-sm mt-1">{{ $message }}</p> @enderror
            </x-field>
        </div>
    </section>
